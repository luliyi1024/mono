...
BOOL g_use_custom_console = FALSE;
...
void x_log_init()
{
	if (AllocConsole()){
		...
		g_use_custom_console = TRUE;
		hOut = GetStdHandle(STD_OUTPUT_HANDLE);
		SetConsoleOutputCP(936);

		SetConsoleMode(hOut,ENABLE_QUICK_EDIT_MODE | ENABLE_EXTENDED_FLAGS);

		ZeroMemory(&historyInfo,sizeof(historyInfo));
		historyInfo.cbSize = sizeof(historyInfo);
		historyInfo.HistoryBufferSize = 900;
		historyInfo.NumberOfHistoryBuffers = 100;
		SetConsoleHistoryInfo(&historyInfo);

		windowBufferSize.X = 200;
		windowBufferSize.Y = 1350;
		SetConsoleScreenBufferSize(hOut,windowBufferSize);
